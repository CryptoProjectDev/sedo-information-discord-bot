...
_PROGRAM_NAME = "SEDO-discord-price-bot"
_VERSION = "0.2.4"
...
def cmd_help():
    return ("trading commands: `price`  `price <exchange>`  `volume`  `ratio`  `rank`  `btc`  `eth`  `marketcap`\n"
            ...
            + "quick link commands: `SEDO` `whitepaper`  `website`  `ann`  `contract`  `stats`  `miners`  `merch`\n"
...
_GLOBAL_COMMANDS = [
    CmdDef(
        ...
        cmd_help),
    CmdDef(
        ["sedo","S.E.D.O.", "info", "iamnoob", "i am noob", "givemeall", "give me all", "?", "pleasehelp", "please help"],
        ("SEDO Website: <http://sedocoin.org/>\n"
	 "SEDO Whitepaper: <http://sedocoin.org/download/sedo_wp_en.pdf>\n"
	 "[ANN] SEDO: <https://bitcointalk.org/index.php?topic=5061527\n"
	 "SEDO Contract: 0x0F00f1696218EaeFa2D2330Df3D6D1f94813b38f [<https://bit.ly/2KdcDo6>]\n"
	 "SEDO Stats: <http://199.247.0.146:5000/> (GitHub: <https://github.com/CryptoProjectDev/SEDO-Stats>)\n"
	 "SEDOminer: <https://github.com/CryptoProjectDev/SedoMiner/releases>\n"
    ...
    CmdDef(
        ["S.E.D.O.", "info", "iamnoob", "i am noob", "givemeall", "give me all", "?", "pleasehelp", "please help"],
        ("SEDO Website: <http://sedocoin.org/>\n"
	 "SEDO Whitepaper: <http://sedocoin.org/download/sedo_wp_en.pdf>\n"
	 "[ANN] SEDO: <https://bitcointalk.org/index.php?topic=5061527\n"
	 "SEDO Contract: 0x0F00f1696218EaeFa2D2330Df3D6D1f94813b38f [<https://bit.ly/2KdcDo6>]\n"
    ...
    CmdDef(
        ['bounty'],
        "Bounty and other rewards will be anounced later. No plans for the near future."),
    CmdDef(
        ['beer'],
        "Drunk beer, my friend! Crypto is only virtual thing ! :D <https://en.wikipedia.org/wiki/Beer>"),
    CmdDef(
        ['white paper'],
        "SEDO Whitepaper: <http://sedocoin.org/download/sedo_wp_en.pdf>"),
    CmdDef(
...
def cmd_compare_price_vs(item_name="lambo", item_price=200000):
    ...
    return "1 {} = **{}** SEDO (${})".format(item_name, 
...
def cmd_priceall():
    ...
    for api in sorted(apis.alive_apis, key=lambda a: a.api_name):
        # this skips CMC and apis not directly tracking SEDO
...
def check_and_set_top_share(resulting_difficulty, author_name, author_id, digest):
    ...
    if Web3.toInt(digest) <= token.mining_target:
        result += "\n~~~~~"
        result += "\n:money_mouth: You seem to have solved a block!? Try your luck here [<https://etherscan.io/address/0x0F00f1696218EaeFa2D2330Df3D6D1f94813b38f#writeContract>]"
...
def cmd_pools():
    all_pools = (
        ("SEDO Mining Pool", "http://pool.sedocoin.org/", "0xeabe"),
...
def cmd_ratio():
    ...
    return "1 BTC : {:,.0f} SEDO".format(apis.btc_price_usd() / token_price_usd)
...
def convert(amount, src, dest):
    ...
    if result == None:
        return "Bad currency ({}). SEDO, SEDOsatoshis, eth, wei, btc, mbtc, satoshis, and usd are supported.".format(dest)
...
def cmd_convert(message):
    # example input: '!convert 1 usd to SEDO'
    ...
    try:
        ...
        return "Something went wrong :sob: try this: `!convert 1 eth to SEDO`"
    else:
        return convert(amount, src, dest)
    
    # example input: '!convert 1 usd SEDO'
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SEDO Server Price Bot v{}'.format(_VERSION),
                                     epilog='<3 0x1d00ffff')
